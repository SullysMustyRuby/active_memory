# import ActiveMemory.Query, only: [from: 1, from: 2, join: 4, join: 5, distinct: 3, where: 3]

defmodule ActiveMemory.Association.NotLoaded do
  @moduledoc """
  ...
  defimpl Inspect do
    def inspect(not_loaded, _opts) do
      ...
      ~s(#ActiveMemory.Association.NotLoaded<#{msg}>)
    end
  end
end

defmodule ActiveMemory.Association do
  @moduledoc false
  ...
  alias ActiveMemory.Query.Builder.OrderBy
  ...
  Invoked by `ActiveMemory.build_assoc/3`.
  """
  @callback build(t, owner :: ActiveMemory.Schema.t(), %{atom => term} | [Keyword.t()]) ::
              ActiveMemory.Schema.t()
  ...
  inside queries.
  """
  @callback joins_query(t) :: ActiveMemory.Query.t()
  ...
  This callback is used by `ActiveMemory.assoc/2` and when preloading.
  """
  @callback assoc_query(t, ActiveMemory.Query.t() | nil, values :: [term]) ::
              ActiveMemory.Query.t()

  @doc """
  Returns information used by the preloader.
  """
  @callback preload_info(t) ::
              {:assoc, t, {integer, atom} | {integer, atom, ActiveMemory.Type.t()}}
              | {:through, t, [atom]}
  ...
  @callback on_repo_change(
              t,
              parent :: ActiveMemory.Changeset.t(),
              changeset :: ActiveMemory.Changeset.t(),
              ActiveMemory.Adapter.t(),
              Keyword.t()
            ) ::
              {:ok, ActiveMemory.Schema.t() | nil} | {:error, ActiveMemory.Changeset.t()}
  ...
  ## Examples
      ...
      iex> ActiveMemory.Association.association_key(Hello.World, :id)
      :world_id

      iex> ActiveMemory.Association.association_key(Hello.HTTP, :id)
      :http_id

      iex> ActiveMemory.Association.association_key(Hello.HTTPServer, :id)
      :http_server_id
  ...
  the first relationship in the chain. Used in ActiveMemory.assoc/2.
  """
  ...
  defp chain_through(owner, through, join_to, values) do
    ...
    counter = ActiveMemory.Query.Builder.count_binds(query) - 1
    ...
    final_bind = ActiveMemory.Query.Builder.count_binds(query) - 1
  ...
  def combine_assoc_query(%{wheres: []} = query, conditions) do
    ...
    %{
      query
      | wheres: [
          %ActiveMemory.Query.BooleanExpr{
            op: :and,
            expr: expr,
            params: params,
            line: __ENV__.line,
    ...
    {expr, params, _counter} =
      Enum.reduce(conditions, {expr, params, counter}, fn
        ...
        {key, {:fragment, frag}}, {expr, params, counter} when is_binary(frag) ->
          pieces = ActiveMemory.Query.Builder.fragment_pieces(frag, [to_field(binding, key)])
  ...
  ## Examples
      ...
      iex> ActiveMemory.Association.related_from_query({"custom_source", Schema}, :comments_v1)
      Schema

      iex> ActiveMemory.Association.related_from_query(Schema, :comments_v1)
      Schema

      iex> ActiveMemory.Association.related_from_query("wrong", :comments_v1)
  ...
  def merge_source(struct, {source, _}) do
    ActiveMemory.put_meta(struct, source: source)
  end
  ...
  def update_parent_prefix(
      ...
      do: update_in(changeset.data, &ActiveMemory.put_meta(&1, prefix: prefix))
  ...
  defp maybe_replace_one!(
    ...
    if replaceable?(previous) and primary_key!(previous) != primary_key!(current) do
      changeset = %{ActiveMemory.Changeset.change(previous) | action: :replace}
      ...
      case mod.on_repo_change(meta, parent_changeset, changeset, adapter, opts) do
        ...
        {:error, changeset} ->
          raise ActiveMemory.InvalidChangesetError, action: changeset.action, changeset: changeset
      end
    end
  end
  ...
  defp replaceable?(%ActiveMemory.Association.NotLoaded{}), do: false
  defp replaceable?(%{__meta__: %{state: :built}}), do: false
  defp replaceable?(_), do: true

  defp primary_key!(nil), do: []
  defp primary_key!(struct), do: ActiveMemory.primary_key!(struct)
end

defmodule ActiveMemory.Association.Has do
  @moduledoc """
  ...
  @behaviour ActiveMemory.Association
  ...
  def after_compile_validation(%{queryable: queryable, related_key: related_key}, env) do
    compiled = ActiveMemory.Association.ensure_compiled(queryable, env)
    ...
    cond do
      ...
      not function_exported?(queryable, :__schema__, 2) ->
        {:error, "associated module #{inspect(queryable)} is not an ActiveMemory schema"}
  ...
  def struct(module, name, opts) do
    ...
    related = ActiveMemory.Association.related_from_query(queryable, name)
    ...
    unless Module.get_attribute(module, :active_memory_fields)[ref] do
      raise ArgumentError,
    ...
    defaults = ActiveMemory.Association.validate_defaults!(module, name, opts[:defaults] || [])

    preload_order =
      ActiveMemory.Association.validate_preload_order!(name, opts[:preload_order] || [])
    ...
    %__MODULE__{
      ...
      related: related,
      owner_key: ref,
      related_key: opts[:foreign_key] || ActiveMemory.Association.association_key(module, ref),
      queryable: queryable,
      on_delete: on_delete,
  ...
  def joins_query(
    ...
    |> ActiveMemory.Association.combine_joins_query(assoc.where, 1)
  end
  ...
  def assoc_query(%{related_key: related_key, queryable: queryable} = assoc, query, [value]) do
    ...
    |> ActiveMemory.Association.combine_assoc_query(assoc.where)
  end
  ...
  def on_repo_change(
    ...
    try do
      ...
      ActiveMemory.StaleEntryError -> {:ok, nil}
    end
  end
  ...
  def on_repo_change(
    ...
    changeset = ActiveMemory.Association.update_parent_prefix(changeset, parent)
  ...
  defp update_parent_key(changeset, _action, key, value),
    do: ActiveMemory.Changeset.put_change(changeset, key, value)
  ...
  ## Relation callbacks
  @behaviour ActiveMemory.Changeset.Relation

  @impl true
  def build(%{related: related, queryable: queryable, defaults: defaults}, owner) do
    related
    |> ActiveMemory.Association.apply_defaults(defaults, owner)
    |> ActiveMemory.Association.merge_source(queryable)
  end
  ...
  def delete_all(refl, parent, repo_name, opts) do
    if query = on_delete_query(refl, parent) do
      ActiveMemory.Repo.Queryable.delete_all(repo_name, query, opts)
    end
  end
  ...
  def nilify_all(%{related_key: related_key} = refl, parent, repo_name, opts) do
    if query = on_delete_query(refl, parent) do
      ActiveMemory.Repo.Queryable.update_all(repo_name, query, [set: [{related_key, nil}]], opts)
    end
  end
...
defmodule ActiveMemory.Association.HasThrough do
  @moduledoc """
  ...
  def struct(module, name, opts) do
    ...
    refl =
      case through do
        [h, _ | _] ->
          Module.get_attribute(module, :active_memory_assocs)[h]
  ...
  def joins_query(%{owner: owner, through: through}) do
    ActiveMemory.Association.join_through_chain(owner, through, from(x in owner))
  end

  @impl true
  def assoc_query(%{owner: owner, through: through}, _, values) do
    ActiveMemory.Association.filter_through_chain(owner, through, values)
  end
end

defmodule ActiveMemory.Association.BelongsTo do
  @moduledoc """
...
defmodule ActiveMemory.Association.ManyToMany do
  @moduledoc """
  ...
  def after_compile_validation(%{queryable: queryable, join_through: join_through}, env) do
    compiled = ActiveMemory.Association.ensure_compiled(queryable, env)
    join_compiled = ActiveMemory.Association.ensure_compiled(join_through, env)
    ...
    cond do
      ...
      not function_exported?(join_through, :__schema__, 2) ->
        {:error, ":join_through module #{inspect(join_through)} is not an ActiveMemory schema"}
  ...
  def struct(module, name, opts) do
    ...
    unless Module.get_attribute(module, :active_memory_fields)[owner_key] do
      raise ArgumentError,
    ...
    join_defaults =
      ActiveMemory.Association.validate_defaults!(module, name, opts[:join_defaults] || [])
  ...
  defp default_join_keys(module, related) do
    [
      {ActiveMemory.Association.association_key(module, :id), :id},
      {ActiveMemory.Association.association_key(related, :id), :id}
    ]
  end
  ...
  def joins_query(
    ...
    |> ActiveMemory.Association.combine_joins_query(assoc.where, 2)
    |> ActiveMemory.Association.combine_joins_query(assoc.join_where, 1)
  end
  ...
  def assoc_query(assoc, query, values) do
    ...
    # We only need to join in the "join table". Preload and ActiveMemory.assoc expressions can then filter
  ...
  def on_repo_change(
    ...
    query =
      ...
      |> ActiveMemory.Association.combine_assoc_query(join_where)
  ...
  def on_repo_change(
    ...
    changeset = ActiveMemory.Association.update_parent_prefix(changeset, owner)
  ...
  defp insert_join(join_through, refl, parent_changeset, data, opts) when is_atom(join_through) do
    ...
    changeset =
      join_through
      |> ActiveMemory.Association.apply_defaults(refl.join_defaults, owner)
      |> Map.merge(data)
      |> ActiveMemory.Changeset.change()
  ...
  defp dump!(action, join_through, struct, field, adapter) when is_binary(join_through) do
    ...
    case ActiveMemory.Type.adapter_dump(adapter, type, value) do
      {:ok, value} ->
        value

      :error ->
        raise ActiveMemory.ChangeError,
