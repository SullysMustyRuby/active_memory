defmodule ActiveMemory.Operations do
  ...
  When the schema has an auto-uuid field (a `uuid` attribute, or an Ecto schema's
  autogenerated `binary_id` primary key) a value is generated if one is absent.
  When the table has a `ttl` the record's `expires_at` is stamped from the current
  time. Returns `{:error, :bad_schema}` when the struct does not match `table`.
  """
  ...
  defp put_auto_uuid(struct, table) do
    case table.__attributes__(:auto_uuid_field) do
      nil ->
        struct

      field ->
        case Map.get(struct, field) do
          nil -> Map.put(struct, field, Ecto.UUID.generate())
          _set -> struct
        end
    end
  end
