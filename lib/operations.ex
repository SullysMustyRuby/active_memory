defmodule ActiveMemory.Operations do
  ...
  Called by a `Store` or `ActiveRepo` as it starts. Returns `{:ok, :created}` for a
  new table, or `{:ok, :recovered}` when an existing one was reclaimed — an ETS table
  held by `ActiveMemory.TableHeir` across a crash, or a Mnesia table that was already
  loaded.

  The table's configuration is validated first, so a schema that cannot work raises
  here rather than misbehaving later: a `ttl` with no `expires_at` field, a primary
  key ActiveMemory cannot generate or that is not the table key, or a composite
  primary key.
  """
  ...
  Returns `:ok`. This is not transactional with respect to concurrent writes: a
  record written while the clear is in flight may survive it.
  """
  ...
  record, mirroring `c:Ecto.Repo.one/2`. Use `select/3` when many records are
  expected.
  """
  ...
  `action` set to `:insert`, mirroring `c:Ecto.Repo.insert/2` so a Phoenix form
  renders the errors.
